            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredOrders.map((order) => (
                    (() => {
                        ...
                        const dealDiscount = Number(order.dealDiscount || 0);
                        const hasDeals = dealDiscount > 0 || (order.appliedDeals && order.appliedDeals.length > 0);
                        ...
                        return (
                            <div key={order._id} className="bg-white border rounded-lg p-4 shadow-sm text-black">
                                <div className="flex justify-between mb-2">
                                    ...
                                    <span className={`px-2 py-1 text-xs rounded h-fit font-semibold ${order.status === 'Preparing' ? 'bg-yellow-100 text-yellow-800' :
                                ...
                                {hasDeals && (
                                    <div className="text-xs bg-purple-50 text-purple-700 p-2 rounded mb-3 space-y-1">
                                        <div className="font-semibold">Deal applied</div>
                                        {order.appliedDeals && order.appliedDeals.length > 0 && (
                                            <div className="space-y-0.5">
                                                {order.appliedDeals.map((deal, idx) => (
                                                    <div key={idx} className="flex justify-between">
                                                        <span>
                                                            {deal.dealName || "Deal"} · {deal.discountType === 'bogo'
                                                                ? 'BOGO'
                                                                : `${Number(deal.discount || 0)}% OFF`}
                                                        </span>
                                                        {deal.savings && deal.savings > 0 && (
                                                            <span className="text-green-700">-${deal.savings.toFixed(2)}</span>
                                                        )}
                                                    </div>
                                                ))}
                                            </div>
                                ...
                                <div className="flex justify-between items-center mt-2">
                                    <div>
                                        {hasDeals && (
                                            <div className="text-xs text-gray-600">Subtotal: ${subtotal.toFixed(2)}</div>
                                        )}
